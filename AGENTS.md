...
├── quiz/                  # 간격 반복 학습 앱 (GitHub Pages 배포)
│   ├── cards/             # 소스별 퀴즈 카드 JSON 파일
│   ├── scripts/           # 빌드 스크립트 (prepare-cards.js)
│   └── app/               # Vite 기반 웹 앱 (SM-2 알고리즘)
└── .obsidian/             # 옵시디언 설정
```
...
4. **기존 페이지 업데이트** — 새 소스와 관련된 개체·개념·분석 페이지를 갱신
   ...
   - 기존 주장과 모순되면 경고 표시
   - 교차 참조 추가
5. **퀴즈 카드 생성** — `quiz/cards/NNN_slug.json` 에 해당 소스의 퀴즈 카드 생성
   - 카드 유형: `definition`, `causal`, `cloze`, `comparison`, `timeline`, `person`
   - 소스당 최소 8~15장의 카드 생성
   - 한국어 기본, 영어 전문용어 적극 병기
   - 기존 카드와 중복되지 않도록 주의
   - JSON 스키마: `{ source, created, cards: [{ id, type, difficulty, front, back, tags, relatedPages }] }`
